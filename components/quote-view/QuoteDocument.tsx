            ...
            <>
              ...
              {activeTemplate === 'professional' ? (
                ...
                <div className="p-4">
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <div className="flex-1">
                      ...
                      {settings.companyAddress && (
                        <p className="text-[11px] text-slate-600 leading-snug whitespace-pre-line mt-0.5">{settings.companyAddress}</p>
                      )}
                      {settings.phone && <p className="text-[11px] text-slate-600 mt-0.5">{settings.phone}</p>}
                      {settings.email && <p className="text-[11px] text-slate-600">{settings.email}</p>}
                      {settings.vatNumber && <p className="text-[11px] text-slate-600">{settings.vatNumber}</p>}
                    </div>
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-[11px] text-slate-600 font-semibold">{quote.type === 'invoice' ? 'Invoice' : 'Quote'}# {reference}</p>
                      <div className="mt-2">
                        <div className="text-[11px] text-slate-600 font-semibold">Balance Due</div>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-3">
                    <div>
                      <div className="text-[11px] font-bold text-slate-900 mb-1">Bill To</div>
                      ...
                      {resolvedCustomerAddress && <p className="text-[11px] text-slate-600 leading-snug">{resolvedCustomerAddress}</p>}
                    </div>
                          ...
                          <thead>
                            <tr className={tableHeaderStyle}>
                              {templateConfig.showLineNumbers && (
                                <th className="py-2 px-2 text-left w-10 text-[11px] font-semibold">#</th>
                              )}
                              <th className="py-2 px-2 text-left text-[11px] font-semibold">Item & Description</th>
                              {showQtyColumn && (
                                <th className="py-2 px-2 text-center w-16 text-[11px] font-semibold">Qty</th>
                              )}
                              {showRateColumn && (
                                <th className="py-2 px-2 text-right w-24 text-[11px] font-semibold">Rate</th>
                              )}
                              {showAmountColumn && (
                                <th className="py-2 px-2 text-right w-24 text-[11px] font-semibold">Amount</th>
                              )}
                            </tr>
                          </thead>
                        )}
                            ...
                            (item as any).isSubtotal ? (
                              ...
                              <tr key={`subtotal-${idx}`} className="bg-slate-50 border-t border-slate-200">
                                ...
                                <td className="py-1.5 px-2">
                                  <span className={`text-[11px] font-bold uppercase tracking-wider ${item.itemType === 'labour' ? 'text-blue-600' : 'text-amber-600'}`}>
                                    {item.description}
                                  </span>
                                </td>
                                ...
                                {showAmountColumn && (
                                  <td className={`py-1.5 px-2 text-right font-bold text-xs ${item.itemType === 'labour' ? 'text-blue-600' : 'text-amber-600'}`}>
                              ...
                              <tr key={`heading-${idx}`} className="bg-slate-50">
                                <td colSpan={colSpan} className="py-1 px-2">
                                  <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500">{item.description}</span>
                                </td>
                              </tr>
                            ) : activeTemplate === 'spacious' ? (
                              ...
                              <tr key={`item-${idx}`} className={templateConfig.showTableBorders ? 'border-b border-slate-100' : ''}>
                                {templateConfig.showLineNumbers && (
                                  <td className="py-2 px-2 text-slate-600 text-[11px]">{item.lineNum}</td>
                                )}
                                <td className="py-2 px-2 text-[11px]">
                                  <div className="text-slate-900 font-medium">{item.name}</div>
                                  {item.subtext && <div className="text-[10px] text-slate-500 mt-0.5">{item.subtext}</div>}
                                </td>
                                {showQtyColumn && (
                                  <td className="py-2 px-2 text-slate-900 text-[11px] text-center font-medium">{item.qty}</td>
                                )}
                                {showRateColumn && (
                                  <td className="py-2 px-2 text-slate-900 text-[11px] text-right">{item.rate ? `${item.rate.toFixed(2)}` : '-'}</td>
                                )}
                                {showAmountColumn && (
                                  <td className="py-2 px-2 text-slate-900 text-[11px] text-right font-medium">
                                    {item.amount.toFixed(2)}
                                  </td>
                                )}
                              </tr>
                    ...
                    <div key={section.id} className={`${templateStyle.sectionPadding} ${idx % 2 === 1 ? 'bg-slate-50/30' : 'bg-white'} ${templateStyle.sectionSpacing}`}>
                      ...
                      {displayOptions.showMaterialSectionTotal && rawMaterialsTotal > 0 && (
                        <div className={`flex justify-between items-center py-1 ${templateConfig.showBackgrounds ? 'bg-slate-50 px-2 rounded-lg border border-slate-100/50' : 'border-t border-slate-100 px-1'}`}>
                          <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Materials Total</span>
                      ...
                      {displayOptions.showWorkSectionTotal && (
                        <div className="flex justify-between items-center pt-2 border-t border-slate-100">
                          ...
                          <p className="text-[11px] font-bold text-slate-600">Section Total: <span className="text-slate-900 ml-1">£{(sectionTotal * markupMultiplier).toFixed(2)}</span></p>
                        </div>
                      )}
                    </div>
                  );
                ...
                <div className="px-2 py-1">
                  <div className="flex justify-end">
                    <div className="w-48">
                      ...
                      <div className="flex justify-between py-1 font-bold bg-slate-100 px-2 -mx-2 mt-1 rounded text-xs">
                    ...
                    <div className="space-y-1">
                      {displayOptions.showTotalsBreakdown && (
                        <>
                          ...
                          {settings.enableVat && displayOptions.showVat && (
                            <div className={`flex justify-between text-[11px] ${activeTemplate === 'minimal' ? 'text-slate-700 font-medium' : 'opacity-70'}`}><span>VAT ({quote.taxPercent}%)</span><span className={activeTemplate === 'minimal' ? '' : 'opacity-90'}>£{totals.taxAmount.toFixed(2)}</span></div>
                          )}
                          {settings.enableCis && displayOptions.showCis && totals.cisAmount > 0 && (
                            <div className={`flex justify-between text-[11px] ${activeTemplate === 'minimal' ? 'text-slate-700 font-medium' : 'opacity-70'}`}><span>CIS Deduction ({quote.cisPercent}%)</span><span className={activeTemplate === 'minimal' ? '' : 'opacity-90'}>-£{totals.cisAmount.toFixed(2)}</span></div>
                          )}
                        </>
                      )}
                      ...
                      <div className="flex justify-between items-center">
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`font-bold ${activeTemplate === 'minimal' ? 'text-sm' : 'text-[11px] opacity-80'} uppercase tracking-wider`}>{quote.type === 'invoice' ? 'Balance Due' : 'Total Due'}</span>
                        </div>
                        ...
                        <div className="mt-4 pt-4 border-t border-slate-200">
                          <div className="bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-200 rounded-xl p-2">
                            <div className="flex items-center gap-2 mb-2">
                              ...
                              <h4 className="text-[11px] font-black text-emerald-900 uppercase tracking-widest">Payment Details</h4>
                            </div>
              ...
              {quote?.designNotes && (
                <div className="px-4 py-3 border-t border-slate-200">
                  <div className="text-[11px] font-bold text-slate-900 mb-1">Design Specification</div>
                  <div className="text-[11px] text-slate-600 leading-relaxed whitespace-pre-wrap">{quote.designNotes}</div>
                </div>
              )}
              ...
              {activeTemplate === 'professional' && (displayOptions.showNotes || (settings.bankAccountName || settings.bankAccountNumber || settings.bankSortCode || settings.bankName)) && (
                <div className="px-4 py-3 border-t border-slate-200">
                  <div className="text-[11px] font-bold text-slate-900 mb-2">Notes</div>
                  {(settings.bankAccountName || settings.bankAccountNumber || settings.bankSortCode || settings.bankName) && quote.type === 'invoice' && (
                    <div className="mb-2">
                      <div className="text-[11px] text-slate-900">Payment Details:</div>
