...
import { ArrowLeft, Clock, CheckCircle, XCircle, MapPin, Briefcase, Loader2, AlertCircle, Download, Coffee } from 'lucide-react';
...
const calculateNetHours = (clockIn: string, clockOut: string | null, breakMinutes: number) => {
  if (!clockOut) return 0;
  const ms = new Date(clockOut).getTime() - new Date(clockIn).getTime();
  const grossMinutes = ms / 60000;
...
export const TimesheetApproval: React.FC<TimesheetApprovalProps> = ({ onBack }) => {
  ...
  const [dateFrom, setDateFrom] = useState(() => toDateInputValue(getMonday(new Date())));
  const [dateTo, setDateTo] = useState(() => toDateInputValue(getSunday(getMonday(new Date()))));
  ...
  const handleExportCSV = () => {
    ...
    const headers = ['Worker', 'Date', 'Clock In', 'Clock Out', 'Break (mins)', 'Net Hours', 'Job', 'Hourly Rate', 'Pay', 'Status'];
    const rows = filtered.map(ts => {
      const netHrs = calculateNetHours(ts.clock_in, ts.clock_out, ts.break_minutes);
      const rate = ts.team_member?.hourly_rate || 0;
      const pay = rate > 0 ? (netHrs * rate).toFixed(2) : '';
      return [
    ...
    a.href = url;
    a.download = `timesheets_${dateFrom}_${dateTo}.csv`;
    a.click();
    ...
    <div className="px-4 pt-4 lg:px-6 space-y-4 pb-40">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-2 bg-teal-500 text-white text-xs font-semibold rounded-xl shadow-lg shadow-teal-500/20 hover:bg-teal-600 transition-colors"
        >
      ...
      <div className="flex gap-3">
        <div className="flex-1">
          ...
          <input
            ...
            className="w-full py-2 px-3 bg-white border border-slate-200 rounded-xl text-sm text-slate-800 focus:outline-none focus:border-teal-500 focus:ring-2 focus:ring-teal-500/20"
          />
        </div>
        <div className="flex-1">
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-3">
          {filtered.map(ts => {
            const netHrs = calculateNetHours(ts.clock_in, ts.clock_out, ts.break_minutes);
            const rate = ts.team_member?.hourly_rate || 0;
            const pay = rate > 0 ? netHrs * rate : 0;
            ...
            return (
              <div key={ts.id} className="bg-white rounded-2xl border border-slate-200 p-4 space-y-3 shadow-sm hover:shadow-md transition-shadow">
                ...
                {/* Pay calculation */}
                {ts.clock_out && rate > 0 && (
                  <div className="bg-teal-50 border border-teal-100 rounded-xl px-3 py-2 text-xs font-medium text-teal-700">
                    {netHrs.toFixed(1)}h net @ £{rate.toFixed(2)}/hr → <span className="font-bold">£{pay.toFixed(2)}</span>
                  </div>
                )}
