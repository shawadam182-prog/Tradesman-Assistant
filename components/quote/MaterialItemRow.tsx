...
export const MaterialItemRow: React.FC<MaterialItemRowProps> = ({
  ...
  return (
    <div className={`bg-white p-0.5 md:p-2 rounded-md md:rounded-lg shadow-sm border relative group overflow-hidden ${item.isAIProposed ? 'border-l-2 border-l-teal-400 border-slate-100 md:border-slate-200' : 'border-slate-100 md:border-slate-200'}`}>
      {/* Name + Description + Actions row — horizontal layout */}
      <div className="flex items-center gap-1 md:gap-2 mb-0.5 md:mb-1">
        <div className="flex-1 min-w-0 flex items-center gap-1 md:gap-2">
          ...
          <input
            type="text"
            className="flex-1 min-w-0 h-4 md:h-7 text-[8px] md:text-xs text-slate-500 outline-none placeholder:text-slate-300 bg-transparent leading-none md:leading-normal p-0 m-0"
      ...
      {/* Qty / Price / Total — compact row */}
      <div className="flex gap-0.5 md:gap-2 bg-slate-50 p-0.5 md:p-1.5 rounded md:rounded-lg">
        <div className="flex-1">
          ...
          <div className="flex items-center bg-white rounded md:rounded-lg shadow-sm border border-slate-100 h-6 md:h-8">
            ...
            <input
              type="number"
              className="w-8 md:w-14 h-full bg-transparent text-xs md:text-sm font-black text-center outline-none text-slate-900"
              value={item.quantity || ''}
        ...
        <div className="flex-1 flex flex-col justify-end">
          <label className="hidden md:block text-[10px] font-bold text-slate-400 uppercase tracking-wider leading-none mb-0.5">Total</label>
